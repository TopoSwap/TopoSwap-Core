...
import './TopoERC20.sol';
import './interfaces/IERC20.sol';
import './interfaces/ITopoPair.sol';
import './interfaces/ITopoFactory.sol';
import './interfaces/ITopoCallee.sol';
import './libraries/Math.sol';
import './libraries/UQ112x112.sol';

contract TopoPair is ITopoPair, TopoERC20 {
    using SafeMath  for uint;
    using UQ112x112 for uint224;

    uint public constant MINIMUM_LIQUIDITY = 1;
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        { // scope for _token{0,1}, avoids stack too deep errors
            ...
            if (data.length > 0) ITopoCallee(to).swapCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
